...
class Bitfinex {
	...
	public function get_ticker($symbol) {
		$request=$this->build_url_path("ticker", $symbol);
		$ticker=$this->send_public_endpoint_request($request);
		$t=array();
		if (substr($symbol, 0, 1)=="t") {
                            $t["bid"]=number_format($ticker[0],10);
                            $t["bid_size"]=number_format($ticker[1],10);
                            $t["ask"]=number_format($ticker[2],10);
                            $t["ask_size"]=number_format($ticker[3],10);
                            $t["daily_change"]=number_format($ticker[4],10);
                            $t["daily_change_perc"]=$ticker[5];
                            $t["last_price"]=number_format($ticker[6],10);
                            $t["volume"]=$ticker[7];
                            $t["high"]=number_format($ticker[8],10);
                            $t["low"]=number_format($ticker[9],10);
		} elseif (substr($symbol, 0, 1)=="f") {
                            $t["bid"]=number_format($ticker[1],10);
                            $t["bid_size"]=number_format($ticker[2],10);
                            $t["ask"]=number_format($ticker[4],10);
                            $t["ask_size"]=number_format($ticker[6],10);
                            $t["daily_change"]=number_format($ticker[7],10);
                            $t["daily_change_perc"]=$ticker[8];
                            $t["last_price"]=number_format($ticker[9],10);
                            $t["volume"]=$ticker[10];
                            $t["high"]=number_format($ticker[11],10);
                            $t["low"]=number_format($ticker[12],10);
		}
		...
		for ($z=0; $z<count($orders); $z++) {
			if (substr($orders[$z][3], 0, 1)=="t") {
				$sym_fix=substr($orders[$z][3], 1, strlen($orders[$z][3]));
					...
					$o[$orders[$z][0]]["amount"]=number_format($orders[$z][6],10);
					$o[$orders[$z][0]]["amount_orig"]=number_format($orders[$z][7],10);
					$o[$orders[$z][0]]["price"]="$".number_format($orders[$z][16],10);
